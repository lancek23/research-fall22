...
def binarize(first, second):
    ...
    """
    for i in range(len(train_images[0])):
        parameter = test_images[:, i]
        parameter_A = test_images_A[:, i]
        parameter_B = test_images_B[:, i]
        middle = (np.mean(parameter_A) + np.mean(parameter_B)) / 2
        parameter[parameter < middle] = 0
        parameter[parameter >= middle] = 1
        test_images[:, i] = parameter
    """
    ...
    filepathStart = "Digit datasets/binarize-symmetrically/mnist-"
    ...
    # with open("Digit datasets/binarize-symmetrically/mnist-0-1-test-images","wb") as f: pickle.dump(test_images,f)
    # with open("Digit datasets/binarize-symmetrically/mnist-0-1-train-images","wb") as f: pickle.dump(train_images,f)
    # with open("Digit datasets/binarize-symmetrically/mnist-0-1-test-labels","wb") as f: pickle.dump(test_labels,f)
    # with open("Digit datasets/binarize-symmetrically/mnist-0-1-train-labels","wb") as f: pickle.dump(train_labels,f)

    filepathStart = "Digit datasets/binarize-symmetrically/"
